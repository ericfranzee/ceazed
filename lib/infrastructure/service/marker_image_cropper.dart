...
import 'dart:math';
 ...
 class ImageCropperForMarker {
  ...
  Future<BitmapDescriptor> getMarkerBitmap(int size, {String? text}) async {
    ...
    final Paint paint1 = Paint()..color = CustomStyle.black;
    final Paint paint2 = Paint()..color = CustomStyle.white;
    ...
    if (text != null) {
      m.TextPainter painter = m.TextPainter(textDirection: TextDirection.ltr);
      painter.text = m.TextSpan(
        text: text,
        style: m.TextStyle(
            fontSize: size / 3,
  ...
  Canvas _performCircleCrop(Image image, Size size, Canvas canvas) {
    ...
    canvas.drawRRect(
      ...
      Paint()
        ..color = CustomStyle.white.withOpacity(0.75)
